...
/*
    public int cphelper(int[][][] dp, int[][] grid, int row1 , int col1, int row2){
        int col2 = row1+col1-row2;
        if(row1>=grid.length||col1>=grid[0].length||row2>=grid.length||col2>=grid[0].length||grid[row1][col1]==-1||grid[row2][col2]==-1)return Integer.MIN_VALUE;
        if(dp[row1][col1][row2]!=0)return dp[row1][col1][row2];
        if(row1==grid.length-1&&col1==grid[0].length-1&&row2==grid.length-1&&col2==grid[0].length-1)return grid[row1][col1];
        int a = cphelper(dp,grid,row1,col1+1,row2);
        int b = cphelper(dp,grid,row1+1,col1,row2+1);
        int c = cphelper(dp,grid,row1,col1+1,row2+1);
        int d = cphelper(dp,grid,row1+1,col2,row2);
